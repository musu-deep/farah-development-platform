        ...
        <div className="grid w-full gap-14 lg:grid-cols-12 lg:items-end">
          <div className="lg:col-span-8">
            ...
            <h1 className="font-display display-tight max-w-5xl text-[clamp(3.1rem,7.4vw,8.1rem)] font-black">
              {["نحوّل التعقيد", "إلى قرار،", "والقرار إلى أثر."].map((line, index) => <span key={line} className="block overflow-hidden"><motion.span initial={{ y: "115%" }} animate={{ y: 0 }} transition={{ delay: .12 + index * .13, duration: .85, ease: [.16, 1, .3, 1] }} className={`block ${index === 2 ? "text-violet" : ""}`}>{line}</motion.span></span>)}
            </h1>
          </div>
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: .7, duration: .7 }} className="lg:col-span-4 lg:pb-4">
            <p className="max-w-lg text-lg leading-9 text-white/62">نرافق القيادات والكيانات من التشخيص إلى التنفيذ، ونبني استراتيجيات وهياكل وقدرات ومؤشرات تجعل التحول مرئياً وقابلاً للقياس.</p>
